...
def BuildStageNCMake(factory, n,
    ...
    AddCMake(
        ...
        CMAKE_MODULE_LINKER_FLAGS="-flto -fuse-ld=gold -Wunused-command-line-argument",
        CMAKE_SHARED_LINKER_FLAGS="-flto -fuse-ld=gold -Wunused-command-line-argument",
...
def BuildStageNlibcxx(factory, n,
    ...
    AddCMake(
        ...
        __CMAKE_CXX_FLAGS=WithProperties("-DCMAKE_CXX_FLAGS=-Wno-unused-command-line-argument -stdlib=platform -isystem %%(workdir)s/%s/projects/libcxx/include -isystem %%(workdir)s/llvm-project/libcxx/include" % workdir),
        __CMAKE_EXE_LINKER_FLAGS   =WithProperties("-DCMAKE_EXE_LINKER_FLAGS=-stdlib=libc++ -L %%(workdir)s/%s/lib -Wl,-rpath,%%(workdir)s/%s/lib" % (workdir, workdir)),
        __CMAKE_MODULE_LINKER_FLAGS=WithProperties("-DCMAKE_MODULE_LINKER_FLAGS=-stdlib=libc++ -L %%(workdir)s/%s/lib" % (workdir)),
        __CMAKE_SHARED_LINKER_FLAGS=WithProperties("-DCMAKE_SHARED_LINKER_FLAGS=-stdlib=libc++ -L %%(workdir)s/%s/lib" % (workdir)),
...
def BuildStage32N77(factory, n,
    ...
    if n != 3:
        factory.addStep(LitTestCommand(
                name="test_llvmclang",
                command=["make", "LIT_ARGS=-v", "check-all"],
                timeout=60,
                warnOnFailure=True,
                locks=[centos6_lock.access('counting')],
                workdir=workdir,
...
def BuildNinja(
    ...
    if buildLLD or testLLD:
        cmake_args.update({
                'LLVM_EXTERNAL_LLD_SOURCE_DIR': "../llvm-project/lld",
                })

    if testLLVM or testClang or testExtra or testLLD:
...
def get_builders():
    ...
    factory.addStep(LitTestCommand(
            name            = 'stage1_test_clang',
            command         = ["make", "-j77", "-k", "check-clang"],
            description     = ["testing", "clang"],
            descriptionDone = ["test",    "clang"],
            warnOnFailure=True,
            #locks=[centos6_lock.access('counting')],
